/**
 * TS port with minor modifications of https://github.com/jerosoler/waveform-path
 */

export const linearPath = (audioBuffer: AudioBuffer, options: any) => {
  const {
    channel = 0,
...
export const polarPath = (audioBuffer: AudioBuffer, options: any) => {
  const {
    channel = 0,
    samples = audioBuffer.length,
    distance = 50,
        ...
        if (paths[k].minshow <= normalizeData[f][i] && paths[k].maxshow >= normalizeData[f][i]) {
          switch (paths[k].d) {
            ...
            case 'L': {
              ...
              const pos_x =
                left +
                (length * (paths[k].sr / 100) * normalizeDataValue * positive * mirror * invert +
                  distance) *
                  Math.cos(angleStart);
              const pos_y =
                top +
              ...
              const end_pos_x =
                left +
                (length * (paths[k].er / 100) * normalizeDataValue * positive * mirror * invert +
                  distance) *
                  Math.cos(angleEnd);
              const end_pos_y =
                top +
            ...
            case 'C': {
              ...
              const center_pos_x =
                left +
                (length * (paths[k].r / 100) * normalizeDataValue * positive * mirror * invert +
                  distance) *
                  Math.cos(angle);
              const center_pos_y =
                top +
...
const getNormalizeData = (filteredData: number[][]) => {
  ...
  const normalizeData = [];
  for (let i = 0; i < filteredDataLength; i++) {
    const normalizeDataBlock = filteredData[i].map((n) => Math.max(n * maxMultiplier, 0.05));
